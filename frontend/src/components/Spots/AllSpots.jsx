      ...
      {pathname === "/" &&
        spotsArray?.map((ele, idx) => (
          <div
            ...
            onClick={function () {
              navigateTo(`/spots/${ele.id}`);
            }}
          >
            <img className="spot-image" src={ele.previewImage}></img>
            <div className="location-starrating">
              <div className="location">
                {ele.city}, {ele.state}
              </div>
              <div className="star-rating">{ele.avgRating} Stars</div>
            </div>
            <div className="details">
              <div className="price">${ele.price} per night</div>
            </div>
          </div>
        ))}
      {sessionUser &&
        pathname === "/spots/current" &&
        spotsArray
          ?.filter(function (ele) {
